...
from autodist.kernel.synchronization.compressor import Comopressor
...
class GraphTransformer:
    ...
    def transform(self):
        ...
        with context.graph_mode():
            ...
            self._initialize_synchronizers_and_copmpressors()
    ...
    def _initialize_synchronizers_and_copmpressors(self):
        self._synchronizers = {}
        for node in self._strategy.node_config:
            ...
            if partitioner:
                for part in node.part_config:
                    ...
                    self._compressors[part.var_name] = Compressor.create(part.compressor, part.var_name)
            else:
                ...
                self._compressors[node.var_name] = Compressor.create(node.compressor, node.var_name)
        ...
        for compressor in self._compressors.values():
            compressor.assign_cluster_information(
                num_workers=self._num_workers,
                num_replicas=self._num_local_replicas,
                worker_device=local_worker_device,
                worker_id=local_worker_id,
