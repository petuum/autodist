...
class TimeHistory(tf.keras.callbacks.Callback):
    ...
    def __init__(self, batch_size, log_steps):
        ...
        # Logs start of step 1 then end of each step based on log_steps
        # interval.
        self.timestamp_log = []

        # Records the time each epoch takes to run from start to finish of
        # epoch.
...
class ProfilerCallback(tf.keras.callbacks.Callback):
    ...
    def on_batch_begin(self, batch, logs=None):
        if batch == self.start_step_in_epoch and self.should_start:
            ...
            profiler.start()
            tf.compat.v1.logging.info(
                'Profiler started at Step %s', self.start_step)
