...
from utils.modeling.layers import dense_einsum
from utils.modeling.layers import masked_softmax
...
class CachedAttention(Attention):
    ...
    def _update_cache(self, key_tensor, value_tensor, cache, decode_loop_step):
        ...
        if decode_loop_step is not None:
            ...
            indices = tf.reshape(
                tf.one_hot(
                    decode_loop_step, key_seq_dim, dtype=key_tensor.dtype), [
                    1, key_seq_dim, 1, 1])
            key_tensor = cache["key"] + key_tensor * indices
            value_seq_dim = cache["value"].shape.as_list()[1]
            indices = tf.reshape(
                tf.one_hot(
                    decode_loop_step, value_seq_dim, dtype=value_tensor.dtype), [
                    1, value_seq_dim, 1, 1])
    ...
    def call(self, inputs, decode_loop_step=None):
        ...
        # `context_layer` = [B, F, N, H]
        return tf.einsum(
            "BNFT,BTNH->BFNH", attention_probs, value_tensor), cache
