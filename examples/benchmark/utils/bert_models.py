...
from utils.modeling import losses
from utils.modeling import networks
from utils.modeling.networks import bert_classifier
from utils.modeling.networks import bert_pretrainer
from utils.modeling.networks import bert_span_labeler
...
class BertPretrainLossAndMetricLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        lm_label_weights = tf.keras.backend.cast(
            unpacked_inputs[3], tf.float32)
        ...
        batch_shape = tf.slice(
            tf.keras.backend.shape(sentence_labels), [0], [1])
