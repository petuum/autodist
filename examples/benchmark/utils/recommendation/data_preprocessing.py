...
def _filter_index_sort(raw_rating_path, cache_path):
    ...
    if valid_cache:
        ...
        # Map the ids of user and item to 0 based index for following
        # processing
        ...
        # The dataframe does not reconstruct indices in the sort or filter
        # steps.
        df = df.reset_index()

        grouped = df.groupby(movielens.USER_COLUMN, group_keys=False)
        eval_df, train_df = grouped.tail(
            1), grouped.apply(lambda x: x.iloc[:-1])
...
def instantiate_pipeline(dataset,
    ...
    # type: (str, str, dict, typing.Optional[str], bool, typing.Optional[str],
    # bool) -> (int, int, data_pipeline.BaseDataConstructor)
